...
void LoraMesher::initializeLoRa() {
    ...
    if (radio == nullptr) {
        switch (config.module) {
            ...
            //     break;
            case LoraModules::SX1262_MOD:
                ESP_LOGV(LM_TAG, "Using SX1262 module with manual pin setup");

                // Manually set the TTGO T-Beam v1.2 SX1262 LoRa pins
                config.loraCs = 18;     // Chip Select (NSS)
                config.loraIrq = 33;    // DIO1 (Interrupt)
                config.loraRst = 23;    // Reset
                config.loraIo1 = 32;    // BUSY (Required for SX1262)

                // Create the radio instance with manually assigned pins
                radio = new LM_SX1262(18, 33, 23, 32, config.spi);
                break;

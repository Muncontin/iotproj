 ...
 */
 ...
 class ServerCallbacks : public NimBLEServerCallbacks {
     void onConnect(NimBLEServer* pServer, NimBLEConnInfo& connInfo) override {
         //Serial.printf("Client address: %s\n", connInfo.getAddress().toString().c_str()); //Gets client MAC address
         std::string macStr = connInfo.getAddress().toString();  // "24:6F:28:AB:CD:EF"

         // Convert string to 6-byte raw MAC
         sscanf(macStr.c_str(), "%hhx:%hhx:%hhx:%hhx:%hhx:%hhx",
                &BLE_CONN_ID[0], &BLE_CONN_ID[1], &BLE_CONN_ID[2],
                &BLE_CONN_ID[3], &BLE_CONN_ID[4], &BLE_CONN_ID[5]);
     
         // Optional: Print for debugging
         Serial.printf("Client connected. BLE MAC: %02X:%02X:%02X:%02X:%02X:%02X\n",
                       BLE_CONN_ID[0], BLE_CONN_ID[1], BLE_CONN_ID[2],
                       BLE_CONN_ID[3], BLE_CONN_ID[4], BLE_CONN_ID[5]);
     ...
     void onDisconnect(NimBLEServer* pServer, NimBLEConnInfo& connInfo, int reason) override {
        ...
        // Reset the BLE MAC address
        memset(BLE_CONN_ID, 0, sizeof(BLE_CONN_ID));
 ...
 void setupBLEServer(void) {
     ...
     pAdvertising->setName(bleServerName);
